...
class PasswdMgr {
   public:
      ...
      void makeSalt(std::vector<uint8_t> &salt);
      void hashArgon2(std::vector<uint8_t> &ret_hash, std::vector<uint8_t> &ret_salt, const char *passwd, std::vector<uint8_t> *in_salt = NULL);
